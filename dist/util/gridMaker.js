...
function generateGrid(startCoords, jump, radius) {
    /*
...
function getNextRing(focalPoint, jump, ringDistanceInDegreesPreMod) {
     ...
     */
    const degreesLongitudeBetweenGrids = (0, conversions_1.changeInEastWestKMToLongitudeDegrees)(jump, focalPoint.lat, focalPoint.long);
    const degreesLatitudeBetweenGrids = (0, conversions_1.changeInNorthSouthKMToLatitudeDegrees)(jump, focalPoint.lat);
    ...
    const subdivisionsNorthSouth = sideLength / degreesLatitudeBetweenGrids / magicMathAdjustment; // note: expecting integer values
    const subdivisionsEastWest = sideLength / degreesLongitudeBetweenGrids / magicMathAdjustment;
    const topEdge = [];
    const bottomEdge = [];
    const rightEdge = [];
    const leftEdge = [];
